...
def fetch_backup_data_from_database(timestamp_threshold):
...
def send_to_api(folder_name, folder_uid, timestamp):
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # Mengambil timestamp saat ini
    data = {
    ...
    if response.status_code == 200:
        ...
        display_text_on_oled("Data presensi sukses dikirim.", 16)
        time.sleep(1)
        clear_oled_display()    
    ...
    try:
        ...
        while True:
            ...
            if not status_sent:
                ...
                display_text_on_oled("Gagal mendeteksi.", 16)
                time.sleep(1)
                clear_oled_display()    
...
if __name__ == "__main__":
    ...
    fetch_thread = threading.Thread(target=fetch_backup_data_from_database, args=(timestamp_threshold,))
    fetch_thread.daemon = True  # Mengatur thread sebagai daemon sehingga thread ini akan berhenti saat program utama berhenti
    fetch_thread.start()
